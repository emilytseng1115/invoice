...
def _normalize_pdf_item(item: str) -> str:
    """Remove the vendor suffix requested for PDF display values only."""
    return re.sub(r"\s*\(INTEL\)\s*$", "", item, flags=re.IGNORECASE).strip()
